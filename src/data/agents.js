module.exports = [
  {
    id: 1,
    img: '/assets/images/team/Sofa.jpg',
    socialList: [
      {
        icon: 'fa-facebook',
        liveLink: 'https://www.facebook.com/sofia.rusu.9003',
      },
      {
        icon: 'fa-brands fa-tiktok',
        liveLink: 'https://www.tiktok.com/@sofiarusu1',
      },
      {
        icon: 'fa-instagram',
        liveLink: 'https://www.instagram.com/sofia.rusu.33',
      },
      // {
      //   icon: 'fa-pinterest',
      //   liveLink: 'https://www.pinterest.com/',
      // },
      // { icon: 'fa-dribbble', liveLink: 'https://www.dribbble.com/' },
    ],
    name: `Rusu Sofia`,
    type: 'Agent Imobiliar',
    // ratings: 4.5,
    noOfListings: '2',
    // office: '134 456 3210',
    mobile: '+373 79 069 886',
    // fax: '342 654 1258',
    email: 'sofia.mosan@yahoo.com',
    city: 'Chișinău',
  },
  {
    id: 2,
    img: '/assets/images/team/6.jpg',
    socialList: [
      { icon: 'fa-facebook', liveLink: 'https://www.facebook.com/' },
      { icon: 'fa-twitter', liveLink: 'https://www.twitter.com/' },
      {
        icon: 'fa-instagram',
        liveLink: 'https://www.instagram.com/',
      },
      {
        icon: 'fa-pinterest',
        liveLink: 'https://www.pinterest.com/',
      },
      { icon: 'fa-dribbble', liveLink: 'https://www.dribbble.com/' },
    ],
    name: `Kathleen Myers`,
    type: 'Agent',
    ratings: 5,
    noOfListings: '2',
    office: '134 456 3210',
    mobile: '891 456 9874',
    fax: '342 654 1258',
    email: 'pakulla@findhouse.com',
    city: 'Florida',
  },
  {
    id: 3,
    img: '/assets/images/team/7.jpg',
    socialList: [
      { icon: 'fa-facebook', liveLink: 'https://www.facebook.com/' },
      { icon: 'fa-twitter', liveLink: 'https://www.twitter.com/' },
      {
        icon: 'fa-instagram',
        liveLink: 'https://www.instagram.com/',
      },
      {
        icon: 'fa-pinterest',
        liveLink: 'https://www.pinterest.com/',
      },
      { icon: 'fa-dribbble', liveLink: 'https://www.dribbble.com/' },
    ],
    name: `Mariusz Ciesla`,
    type: 'Broker',
    ratings: 3.5,
    noOfListings: '2',
    office: '134 456 3210',
    mobile: '891 456 9874',
    fax: '342 654 1258',
    email: 'pakulla@findhouse.com',
    city: 'Los Angeles',
  },
  {
    id: 4,
    img: '/assets/images/team/8.jpg',
    socialList: [
      { icon: 'fa-facebook', liveLink: 'https://www.facebook.com/' },
      { icon: 'fa-twitter', liveLink: 'https://www.twitter.com/' },
      {
        icon: 'fa-instagram',
        liveLink: 'https://www.instagram.com/',
      },
      {
        icon: 'fa-pinterest',
        liveLink: 'https://www.pinterest.com/',
      },
      { icon: 'fa-dribbble', liveLink: 'https://www.dribbble.com/' },
    ],
    name: `Helene Powers`,
    type: 'Broker',
    ratings: 4.5,
    noOfListings: '2',
    office: '134 456 3210',
    mobile: '891 456 9874',
    fax: '342 654 1258',
    email: 'pakulla@findhouse.com',
    city: 'Miami',
  },
  {
    id: 5,
    img: '/assets/images/team/9.jpg',
    socialList: [
      { icon: 'fa-facebook', liveLink: 'https://www.facebook.com/' },
      { icon: 'fa-twitter', liveLink: 'https://www.twitter.com/' },
      {
        icon: 'fa-instagram',
        liveLink: 'https://www.instagram.com/',
      },
      {
        icon: 'fa-pinterest',
        liveLink: 'https://www.pinterest.com/',
      },
      { icon: 'fa-dribbble', liveLink: 'https://www.dribbble.com/' },
    ],
    name: `Jade Northon`,
    type: 'Agent',
    ratings: 4.5,
    noOfListings: '2',
    office: '134 456 3210',
    mobile: '891 456 9874',
    fax: '342 654 1258',
    email: 'pakulla@findhouse.com',
    city: 'New York',
  },
  {
    id: 6,
    img: '/assets/images/team/10.jpg',
    socialList: [
      { icon: 'fa-facebook', liveLink: 'https://www.facebook.com/' },
      { icon: 'fa-twitter', liveLink: 'https://www.twitter.com/' },
      {
        icon: 'fa-instagram',
        liveLink: 'https://www.instagram.com/',
      },
      {
        icon: 'fa-pinterest',
        liveLink: 'https://www.pinterest.com/',
      },
      { icon: 'fa-dribbble', liveLink: 'https://www.dribbble.com/' },
    ],
    name: `Kevin Harris`,
    type: 'Agent',
    ratings: 3.5,
    noOfListings: '2',
    office: '134 456 3210',
    mobile: '891 456 9874',
    fax: '342 654 1258',
    email: 'pakulla@findhouse.com',
    city: 'Orlando',
  },
  {
    id: 7,
    img: '/assets/images/team/7.jpg',
    socialList: [
      { icon: 'fa-facebook', liveLink: 'https://www.facebook.com/' },
      { icon: 'fa-twitter', liveLink: 'https://www.twitter.com/' },
      {
        icon: 'fa-instagram',
        liveLink: 'https://www.instagram.com/',
      },
      {
        icon: 'fa-pinterest',
        liveLink: 'https://www.pinterest.com/',
      },
      { icon: 'fa-dribbble', liveLink: 'https://www.dribbble.com/' },
    ],
    name: `Mariusz Ciesla`,
    type: 'Broker',
    ratings: 3.5,
    noOfListings: '2',
    office: '134 456 3210',
    mobile: '891 456 9874',
    fax: '342 654 1258',
    email: 'pakulla@findhouse.com',
    city: 'Atlanta',
  },
  {
    id: 8,
    img: '/assets/images/team/11.jpg',
    socialList: [
      { icon: 'fa-facebook', liveLink: 'https://www.facebook.com/' },
      { icon: 'fa-twitter', liveLink: 'https://www.twitter.com/' },
      {
        icon: 'fa-instagram',
        liveLink: 'https://www.instagram.com/',
      },
      {
        icon: 'fa-pinterest',
        liveLink: 'https://www.pinterest.com/',
      },
      { icon: 'fa-dribbble', liveLink: 'https://www.dribbble.com/' },
    ],
    name: `Christopher Pakulla`,
    type: 'Broker',
    ratings: 3.5,
    noOfListings: '3',
    office: '134 456 3210',
    mobile: '891 456 9874',
    fax: '342 654 1258',
    email: 'pakulla@findhouse.com',
    city: 'Florida',
  },
  {
    id: 9,
    img: '/assets/images/team/12.jpg',
    socialList: [
      { icon: 'fa-facebook', liveLink: 'https://www.facebook.com/' },
      { icon: 'fa-twitter', liveLink: 'https://www.twitter.com/' },
      {
        icon: 'fa-instagram',
        liveLink: 'https://www.instagram.com/',
      },
      {
        icon: 'fa-pinterest',
        liveLink: 'https://www.pinterest.com/',
      },
      { icon: 'fa-dribbble', liveLink: 'https://www.dribbble.com/' },
    ],
    name: `Anna Harrison`,
    type: 'Agent',
    ratings: 3.5,
    noOfListings: '2',
    office: '134 456 3210',
    mobile: '891 456 9874',
    fax: '342 654 1258',
    email: 'pakulla@findhouse.com',
    city: 'Los Angeles',
  },
  {
    id: 10,
    img: '/assets/images/team/13.jpg',
    socialList: [
      { icon: 'fa-facebook', liveLink: 'https://www.facebook.com/' },
      { icon: 'fa-twitter', liveLink: 'https://www.twitter.com/' },
      {
        icon: 'fa-instagram',
        liveLink: 'https://www.instagram.com/',
      },
      {
        icon: 'fa-pinterest',
        liveLink: 'https://www.pinterest.com/',
      },
      { icon: 'fa-dribbble', liveLink: 'https://www.dribbble.com/' },
    ],
    name: `Luxury Family Home`,
    type: 'Broker',
    ratings: 3.5,
    noOfListings: '4',
    office: '134 456 3210',
    mobile: '891 456 9874',
    fax: '342 654 1258',
    email: 'pakulla@findhouse.com',
    city: 'Miami',
  },
  {
    id: 14,
    img: '/assets/images/team/14.jpg',
    socialList: [
      { icon: 'fa-facebook', liveLink: 'https://www.facebook.com/' },
      { icon: 'fa-twitter', liveLink: 'https://www.twitter.com/' },
      {
        icon: 'fa-instagram',
        liveLink: 'https://www.instagram.com/',
      },
      {
        icon: 'fa-pinterest',
        liveLink: 'https://www.pinterest.com/',
      },
      { icon: 'fa-dribbble', liveLink: 'https://www.dribbble.com/' },
    ],
    name: `Mariusz Ciesla`,
    type: 'Agent',
    ratings: 3.5,
    noOfListings: '3',
    office: '134 456 3210',
    mobile: '891 456 9874',
    fax: '342 654 1258',
    email: 'pakulla@findhouse.com',
    city: 'New York',
  },
  {
    id: 15,
    img: '/assets/images/team/15.jpg',
    socialList: [
      { icon: 'fa-facebook', liveLink: 'https://www.facebook.com/' },
      { icon: 'fa-twitter', liveLink: 'https://www.twitter.com/' },
      {
        icon: 'fa-instagram',
        liveLink: 'https://www.instagram.com/',
      },
      {
        icon: 'fa-pinterest',
        liveLink: 'https://www.pinterest.com/',
      },
      { icon: 'fa-dribbble', liveLink: 'https://www.dribbble.com/' },
    ],
    name: `Renovated Apartment`,
    type: 'Broker',
    ratings: 3.5,
    noOfListings: '1',
    office: '134 456 3210',
    mobile: '891 456 9874',
    fax: '342 654 1258',
    email: 'pakulla@findhouse.com',
    city: 'Orlando',
  },
  {
    id: 16,
    img: '/assets/images/team/16.jpg',
    socialList: [
      { icon: 'fa-facebook', liveLink: 'https://www.facebook.com/' },
      { icon: 'fa-twitter', liveLink: 'https://www.twitter.com/' },
      {
        icon: 'fa-instagram',
        liveLink: 'https://www.instagram.com/',
      },
      {
        icon: 'fa-pinterest',
        liveLink: 'https://www.pinterest.com/',
      },
      { icon: 'fa-dribbble', liveLink: 'https://www.dribbble.com/' },
    ],
    name: `Mariusz Ciesla`,
    type: 'Broker',
    ratings: 3.5,
    noOfListings: '2',
    office: '134 456 3210',
    mobile: '891 456 9874',
    fax: '342 654 1258',
    email: 'pakulla@findhouse.com',
    city: 'Orlando',
  },
];
